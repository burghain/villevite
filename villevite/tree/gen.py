...
def calc_tangent_to_bezier(offset, start_point, end_point):
    ...
    res = 3 * one_minus_offset ** 2 * (start_handle_right - start_point.co) + 6 * one_minus_offset * offset *\
        (end_handle_left - start_handle_right) + 3 * \
        offset ** 2 * (end_point.co - end_handle_left)
...
class Stem(object):
    ...
    def copy(self):
        ...
        new_stem = Stem(self.depth, self.curve, self.parent,
                        self.offset, self.radius_limit)
...
class Tree(object):
    ...
    def points_for_floor_split(self):
        ...
        for _ in range(self.param.branches[0]):
            ...
            while not point_ok:
                ...
                dis = sqrt(random_random(
                ) * self.param.branches[0] / 2.5 * self.param.g_scale * self.param.ratio)
    ...
    def create_branches(self):
        ...
        for ind in range(self.param.branches[0]):
            self.tree_scale = self.param.g_scale + \
                random_uniform(-1, 1) * self.param.g_scale_v
            turtle = CHTurtle()
    ...
    def make_leaf(self, leaf, base_leaf_shape, index, verts_array, faces_array):
        ...
        verts, faces = leaf.get_mesh(
            self.param.leaf_bend, base_leaf_shape, index)
    ...
    def make_stem(self, turtle, stem, start=0, split_corr_angle=0, num_branches_factor=1, clone_prob=1,
        ...
        if start == 0:
            stem.length_child_max = self.param.length[d_plus_1] + \
                random_uniform(-1, 1) * self.param.length_v[d_plus_1]
            stem.length = self.calc_stem_length(stem)
        ...
        if cloned_turtle is None and self.param.prune_ratio > 0:
            ...
            # reduction
            in_pruning_envelope = self.test_stem(
                CHTurtle(turtle), stem, start, split_corr_angle, clone_prob)
            ...
            stem.length = start_length * \
                (1 - self.param.prune_ratio) + \
                fitting_length * self.param.prune_ratio
        ...
        # calc base segment
        base_seg_ind = ceil(
            self.param.base_size[0] * int(self.param.curve_res[0]))
            ...
            if self.param.curve_v[depth] < 0:
                ...
                if seg_ind == 0:
                    ...
                    if seg_ind == 1:
                        ...
                        new_point.co = hel_p_2.rotated(
                            Quaternion(hel_axis, (seg_ind - 1) * pi))
                        new_point.co += prev_point.co
                        dif_p = (
                            hel_p_2 - hel_p_1).rotated(Quaternion(hel_axis, (seg_ind - 1) * pi))
                ...
                if cloned_turtle and seg_ind == start:
                    new_point.handle_left = turtle.pos - \
                        cloned_turtle.dir * (stem.length / (curve_res * 3))
                    new_point.handle_right = turtle.pos + \
                        cloned_turtle.dir * (stem.length / (curve_res * 3))
                else:
                    new_point.handle_left = turtle.pos - \
                        turtle.dir * stem.length / (curve_res * 3)
                    new_point.handle_right = turtle.pos + \
                        turtle.dir * stem.length / (curve_res * 3)
            ...
            if seg_ind > start:
                ...
                if self.param.curve_v[depth] >= 0:
                    ...
                    if self.param.base_splits > 0 and depth == 0 and seg_ind == base_seg_ind:
                        ...
                        if self.param.base_splits < 0:
                            num_of_splits = int(
                                random_random() * (abs(self.param.base_splits) + 0.5))
                        else:
                        ...
                        if random_random() <= clone_prob:
                            num_of_splits = int(
                                seg_splits + self.split_num_error[depth])
                            self.split_num_error[depth] -= num_of_splits - \
                                seg_splits
                ...
                if abs(branch_count) > 0 and depth < self.param.levels - 1:
                    ...
                    if abs(branches_on_seg) > 0:
                        self.make_branches(
                            turtle, stem, seg_ind, branches_on_seg, prev_rotation_angle)
                    ...
                    if abs(leaves_on_seg) > 0:
                        self.make_leaves(turtle, stem, seg_ind,
                                         leaves_on_seg, prev_rotation_angle)
                ...
                if self.param.curve_v[depth] >= 0:
                    if num_of_splits > 0:
                        ...
                        is_base_split = (
                            self.param.base_splits > 0 and depth == 0 and seg_ind == base_seg_ind)
                        ...
                        if not is_base_split and num_of_splits == 1:
                            if using_direct_split:
                                ...
                                turtle.dir.rotate(Quaternion(
                                    Vector([0, 0, 1]), radians(-spr_angle / 2)))
                                turtle.dir.normalize()
                                turtle.right.rotate(Quaternion(
                                    Vector([0, 0, 1]), radians(-spr_angle / 2)))
                                turtle.right.normalize()
                    else:
                        # just apply curve and split correction
                        turtle.turn_left(random_uniform(-1, 1)
                                         * self.param.bend_v[depth] / curve_res)
                ...
                if points_per_seg > 2:
                    self.increase_bezier_point_res(
                        stem, seg_ind, points_per_seg)
            ...
            if self.param.curve_v[depth] < 0:
                ...
                if seg_ind == 0:
                    ...
                    if seg_ind == 1:
                        ...
                        hel_p_2.rotate(Quaternion(
                            hel_axis, (seg_ind - 1) * pi))
            ...
            if seg_ind > start:
                ...
                if self.param.curve_v[depth] >= 0:
                    ...
                    if self.param.base_splits > 0 and depth == 0 and seg_ind == base_seg_ind:
                        ...
                        num_of_splits = int(
                            random_random() * (self.param.base_splits + 0.5))
                    elif seg_splits > 0 and seg_ind < curve_res and (depth > 0 or seg_ind > base_seg_ind):
                        # otherwise get number of splits from seg_splits and use Floyd-Steinberg to
                        # fix non-integer values only clone with probability clone_prob
                        if random_random() <= clone_prob:
    ...
    def make_clones(self, turtle, seg_ind, split_corr_angle, num_branches_factor, clone_prob,
        ...
        for split_index in range(num_of_splits):
            ...
            if is_base_split and not using_direct_split:
                eff_spr_angle = (
                    split_index + 1) * (360 / (num_of_splits + 1)) + random_uniform(-1, 1) * stem_depth
    ...
    def make_branches(self, turtle, stem, seg_ind, branches_on_seg, prev_rotation_angle, is_leaves=False):
            ...
            if branch_dist > 1:  # whorled branches
                ...
                for branch_ind in range(branches_on_seg):
                    ...
                    if branch_ind % 2 == 0:
                        ...
                        offset = min(
                            max(0, (branch_ind - branch_dist) / branches_on_seg), 1)
        ...
        if is_leaves:
            for pos_tur, dir_tur, _, _ in branches_array:
                self.leaves_array.append(
                    Leaf(pos_tur.pos, dir_tur.dir, dir_tur.right))
        else:
            for pos_tur, dir_tur, rad, b_offset in branches_array:
                ...
                self.make_stem(dir_tur, Stem(d_plus_1, new_spline,
                               stem, b_offset, rad), pos_corr_turtle=pos_tur)
    ...
    def set_up_branch(self, turtle, stem, branch_mode, offset, start_point,
        ...
        if branch_mode is BranchMode.fan:
            ...
            radius_limit = self.radius_at_offset(
                stem, stem_offset / stem.length)
    ...
    def calc_stem_length(self, stem):
        ...
        if stem.depth == 0:  # trunk
            result = self.tree_scale * \
                (self.param.length[0] + random_uniform(-1, 1)
            ...
            result = stem.parent.length_child_max * \
    ...
    def calc_down_angle(self, stem, stem_offset):
        ...
        if self.param.down_angle_v[d_plus_1] >= 0:
            d_angle = self.param.down_angle[d_plus_1] + \
                random_uniform(-1, 1) * self.param.down_angle_v[d_plus_1]
        else:
    ...
    def calc_rotate_angle(self, depth, prev_angle):
        ...
        if self.param.rotate[depth] >= 0:
            r_angle = (
                prev_angle + self.param.rotate[depth] + random_uniform(-1, 1) * self.param.rotate_v[depth]) % 360
        else:
            r_angle = prev_angle * \
                (180 + self.param.rotate[depth] +
                 random_uniform(-1, 1) * self.param.rotate_v[depth])
        return r_angle
    ...
    def radius_at_offset(self, stem, z_1):
        ...
        if n_taper < 1:
            ...
            if n_taper < 2 and z_3 >= taper:
                ...
                radius = (1 - depth) * taper + depth * \
                    sqrt(pow(taper, 2) - pow((z_3 - taper), 2))
        if stem.depth == 0:
            y_val = max(0, 1 - 8 * z_1)
    ...
    def increase_bezier_point_res(self, stem, seg_ind, points_per_seg):
        ...
        FakeSplinePoint = namedtuple(
            'FakeSplinePoint', ['co', 'handle_left', 'handle_right'])
    ...
    def point_inside(self, point):
        ...
        ratio = (self.tree_scale - point.z) / \
            (self.tree_scale * (1 - self.param.base_size[0]))
        inside = (dist / self.tree_scale) < (self.param.prune_width *
                                             self.shape_ratio(8, ratio))
...
def make_branch_dir_turtle(turtle, helix, offset, start_point, end_point):
    ...
    if helix:
        ...
        tan_d = calc_tangent_to_bezier(
            offset + 0.0001, start_point, end_point).normalized()
        ...
        branch_dir_turtle.right = turtle.dir.cross(
            turtle.right).cross(branch_dir_turtle.dir)
